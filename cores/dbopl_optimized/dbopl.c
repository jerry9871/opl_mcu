/*
    ...
    MCU-optimized variant of cores/dbopl/.
...
/*  ===========================================================================
    MCU optimizations (this is the dbopl_optimized variant)
    ===========================================================================
    Same approach as cores/nuked_optimized/ -- a thin wrapper of attribute
    macros around the upstream port that:

        HOT_INLINE forces gcc to specialise hot helpers per call site even
        at -O2.  Without it, block_body (called via SynthHandler function
        pointer) is inlined but its inner helpers stay out-of-line and the
        per-sample indirect-call overhead dominates -- adds ~30%% runtime
        vs the C++ template original.

        HOT_FUNC places hot non-inline functions in `.ccm` (Core-Coupled
        Memory) so they execute from CCM, avoiding flash-bus contention
        with DMA / interrupt fetches.  Used for:
          - per-sample top-level loops (chip_generate_block2/3)
          - the function-pointer dispatch targets (vol_*, synth_2am/2fm/...)
            which the compiler cannot inline through the indirect call
          - gen_percussion (per-sample if percussion mode is active)
          - op_set_state (called whenever envelope state transitions)

        On targets without CCM (or non-GCC toolchains) HOT_FUNC degrades
        to a no-op and the function lands in flash like normal.  Override
        at build time with -DHOT_FUNC=...  (or -DHOT_FUNC= to disable),
        or use the legacy -DDBOPL_NO_CCM to force the no-op fallback
        without overriding the macro. */
#if defined(__GNUC__) || defined(__clang__)
	#define HOT_INLINE static inline __attribute__((always_inline))
	#ifndef HOT_FUNC
		#if defined(DBOPL_NO_CCM)
			#define HOT_FUNC
		#else
