...
main(int argc, char** argv)
{
	...
	if (use_hs) {
		/*  Heatshrink the codemap+stream payload directly.  All the
		    metadata the MCU needs (codemap_len, short/long codes,
		    total_ms) is baked into the opl_song_hs struct below, so
		    the 26-byte DRO file header would be dead weight inside
		    the compressed stream.  The PC-only .dro_hs runtime
		    loader (demo_win/dro_load.c) keeps the header on disk
		    because that file format must remain self-describing. */
		size_t   cap = total + 1024;
		uint8_t* hs  = malloc(cap);
		size_t   hs_len = hs_encode(out, total, hs_w, hs_l, hs, cap);
		...
		fprintf(g,
				...
				" * Plain payload   : %zu bytes (codemap=%u + stream=%u)\n"
		...
		printf("Wrote %s: %u writes, %u ms, %zu bytes plain -> %zu bytes hs (%.1f%%, w=%d l=%d).\n",
			   out_path, lp, lm, total, hs_len,
			   100.0 * hs_len / total, hs_w, hs_l);
